<?php
    ...
    if (isset($_POST['submit_crear'])) {
        ...
        $tipo_bordado = $_POST['tipo_bordado'];
        $precio_bordado = $_POST['precio_bordado'];
        
        // Utilizar NOW() para obtener la fecha y hora actuales
        $fecha_actualizacion = date('Y-m-d'); // Formatear la fecha
    
        $consulta = "INSERT INTO bordado (tipo_bordado, precio_bordado, fecha_actualizacion) 
        VALUES ('$tipo_bordado', '$precio_bordado', '$fecha_actualizacion')";
        $resultado = mysqli_query($enlace, $consulta);
        header("Location: bordado.php");
        exit();
    }
    
    
    if (isset($_POST['submit_editar'])) {
        $consulta = "UPDATE bordado SET tipo_bordado = '$tipo_bordado', precio_bordado = '$precio_bordado', fecha_actualizacion = NOW()
        WHERE id_bordado = '$id_bordado'";
        $resultado = mysqli_query($enlace, $consulta);
        header("Location: bordado.php");
        exit();
    }
    ...
    if (isset($_POST['submit_eliminar'])) {
        $consulta = "DELETE FROM bordado WHERE id_bordado = '$id_bordado'";
        $resultado = mysqli_query($enlace, $consulta);
        header("Location: bordado.php");
        exit();
    }
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        <div id="content">
            ...
            <div class="text-center mt-3">
                <h1 style="font-family: 'Times New Roman'">Tipos de Bordado</h1>
            </div>
            <!-- DataTable -->
                ...
                <div class="card-body">
                    <d class="row">
                                ...
                                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                    <div class="modal-content rounded-4">
                                        <div class="modal-header" style="background-color: #000DD3;">
                                            <h5 class="modal-title text-white" id="exampleModalLabel">Ingresa los datos del Bordado</h5>
                                        ...
                                        <form action="" method="post" id="formulario" enctype="multipart/form-data">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control" name="tipo_bordado" placeholder="Ingresa el Nombre del Insumo" pattern="[A-Za-z0-9,\sáéíóúÁÉÍÓÚñÑ]+" minlength="3" maxlength="100" required>
                                            </div>

                                            <div class="mb-3">
                                                <label class="form-label" style="color: #000000;">Ingrese el precio del insumo:</label>
                                                <div class="input-group">
                                                    <input type="number" class="form-control" name="precio_bordado" placeholder="Ingrese el Precio" pattern="[0-9]+" minlength="1" maxlength="11" required>
                                                </div>
                                            </div>
                        ...
                        <table id="mytabla" class="table table-striped table-bordered" style="width:100%">
                            <thead>
                                <tr>
                                    <th><center>Tipo de Bordado</center></th>
                            ...
                            <tbody>
                                <?php
                                $consulta = "SELECT bordado.id_bordado, bordado.tipo_bordado, bordado.precio_bordado, bordado.fecha_actualizacion FROM bordado WHERE bordado.id_bordado > 0 ORDER BY bordado.id_bordado";
                                ...
                                while ($fila = mysqli_fetch_array($resultado)) {
                                    ?>
                                    <tr>
                                        <td><center><?php echo $fila['tipo_bordado']; ?></center></td>
                                        <td><center><?php $precio = $fila['precio_bordado']; $precio_formateado = $precio == intval($precio) ? number_format($precio, 0, ',', '.') : number_format($precio, 2, ',', '.');?>$<?= $precio_formateado ?></center></td>
                                        <td><center><?php echo $fila['fecha_actualizacion']; ?></center></td>
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                <button type="button" class="btn btn-warning me-2" data-bs-toggle="modal" data-bs-target="#modalEditar<?php echo $fila['id_bordado']; ?>">
                                                    <i class="bi bi-pencil-square"></i>
                                                </button>
                                                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#modalEliminar<?php echo $fila['id_bordado']; ?>">
                    ...
                    <?php
                        ...
                        ?>
                        <!-- Modal Editar -->
                        <div class="modal fade" id="modalEditar<?php echo $fila['id_bordado']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                <div class="modal-content rounded-4">
                                    <div class="modal-header" style="background-color: #000DD3;">
                                    ...
                                    <div class="modal-body">
                                        <form action="" method="post" id="formulario" enctype="multipart/form-data">
                                            <input type="hidden" name="id_bordado" value="<?php echo $fila['id_bordado']; ?>">
                                            <div class="mb-3">
                                                <label class="form-label" style="color: #000000;">Ingrese el nombre del insumo:</label>
                                                <input type="text" class="form-control" name="tipo_bordado" value="<?php echo $fila['tipo_bordado']; ?>" pattern="[A-Za-z0-9,\sáéíóúÁÉÍÓÚñÑ]+" minlength="3" maxlength="100" required>
                                            </div>

                                            <div class="mb-3">
                                                <label class="form-label" style="color: #000000;">Ingrese el precio del insumo:</label>
                                                <div class="input-group">
                                                    <input type="number" class="form-control" name="precio_bordado" value="<?php echo $fila['precio_bordado']; ?>" pattern="[0-9]+" minlength="1" maxlength="11" required>
                                                </div>
                                            </div>
                        ...
                        <!-- Modal Eliminar -->                   
                        <div class="modal fade" id="modalEliminar<?php echo $fila['id_bordado']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content rounded-4">
                                    <div class="modal-header text-white rounded-top" style="background-color: #000DD3;">
                                        <h5 class="modal-title" id="exampleModalLabel" style="color: white; text-align: center;">¿Realmente desea eliminar el bordado: <?php echo $fila['tipo_bordado']; ?>?</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                                    </div>
                                    <div class="modal-body">
                                        <div class="alert alert-warning" role="alert">
